...
class FlowSimulator:
    ...
    def start(self):
        ...
        log.info("Using ingress nodes list {}\n".format(self.params.ing_nodes))
        log.info("Using egress nodes list {}\n".format(self.params.eg_nodes))
        for node_id in self.params.ing_nodes:
    ...
    def generate_flow(self, node_id):
        ...
        while True:
            ...
            if self.params.sim_config['flow_parameter_mode'] == 'probabilistic_continuous':
                # set normally distributed flow data rate
                flow_dr = np.random.normal(self.params.sim_config['flow_dr_mean'],
                                           self.params.sim_config['flow_dr_stdev'])
                # Exponentially distributed random inter arrival rate
                inter_arr_time = random.expovariate(self.params.sim_config['inter_arrival_lambda'])
                # heavy-tail flow size
                flow_size = np.random.pareto(self.params.sim_config['flow_size_alpha']) + 1

            elif self.params.sim_config['flow_parameter_mode'] == 'probabilistic_discrete':
            ...
            self.metrics.generated_flow(flow)
